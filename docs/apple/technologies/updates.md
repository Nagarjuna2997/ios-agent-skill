# Updates

## Context

Load this when a task names **Updates** or one of the API topics below.

Apple categories: Web.

[Apple documentation](https://developer.apple.com/documentation/updates) · Source checked: 2026-09-16.

Apple’s short description (excerpt):

> Latest additions to documentation and platform release notes.

## Pattern

Use the topic map below to select the API for the requested feature. Follow the [technology implementation workflow](../technology-workflow.md) before writing integration code.

Documented modules: `Updates`.

Documentation language identifiers: swift.

Apple’s directory does not supply platform availability for this entry. It may be a web API, tool, service, resource, or archived technology; inspect its source before choosing a target.

### Release notes

- [iOS & iPadOS Release Notes](https://developer.apple.com/documentation/ios-ipados-release-notes)
- [macOS Release Notes](https://developer.apple.com/documentation/macos-release-notes)
- [tvOS Release Notes](https://developer.apple.com/documentation/tvos-release-notes)
- [watchOS Release Notes](https://developer.apple.com/documentation/watchos-release-notes)
- [visionOS Release Notes](https://developer.apple.com/documentation/visionos-release-notes)
- [Xcode Release Notes](https://developer.apple.com/documentation/xcode-release-notes)
- [Safari Release Notes](https://developer.apple.com/documentation/safari-release-notes)

### Technology and frameworks

- [Accelerate updates](https://developer.apple.com/documentation/updates/accelerate)
- [Accessibility updates](https://developer.apple.com/documentation/updates/accessibility)
- [ActivityKit updates](https://developer.apple.com/documentation/updates/activitykit)
- [AdAttributionKit Updates](https://developer.apple.com/documentation/updates/adattributionkit)
- [App Clips updates](https://developer.apple.com/documentation/updates/appclips)
- [App Intents updates](https://developer.apple.com/documentation/updates/appintents)
- [AppKit updates](https://developer.apple.com/documentation/updates/appkit)
- [Apple Intelligence updates](https://developer.apple.com/documentation/updates/apple-intelligence)
- [AppleMapsServerAPI Updates](https://developer.apple.com/documentation/updates/applemapsserverapi)
- [Apple Pencil updates](https://developer.apple.com/documentation/updates/applepencil)
- [ARKit updates](https://developer.apple.com/documentation/updates/arkit)
- [Audio Toolbox updates](https://developer.apple.com/documentation/updates/audiotoolbox)
- [AuthenticationServices updates](https://developer.apple.com/documentation/updates/authenticationservices)
- [AVFAudio updates](https://developer.apple.com/documentation/updates/avfaudio)
- [AVFoundation updates](https://developer.apple.com/documentation/updates/avfoundation)
- [AVKit updates](https://developer.apple.com/documentation/updates/avkit)
- [Background Tasks updates](https://developer.apple.com/documentation/updates/backgroundtasks)
- [Bundle Resources updates](https://developer.apple.com/documentation/updates/bundleresources)
- [CallKit updates](https://developer.apple.com/documentation/updates/callkit)
- [ContactsUI updates](https://developer.apple.com/documentation/updates/contactsui)
- [Core Location updates](https://developer.apple.com/documentation/updates/corelocation)
- [Core MIDI updates](https://developer.apple.com/documentation/updates/coremidi)
- [Core ML updates](https://developer.apple.com/documentation/updates/coreml)
- [Core Motion updates](https://developer.apple.com/documentation/updates/coremotion)
- [Core Spotlight updates](https://developer.apple.com/documentation/updates/corespotlight)
- [DataDetection updates](https://developer.apple.com/documentation/updates/datadetection)
- [Default apps updates](https://developer.apple.com/documentation/updates/defaultapps)
- [DockKit updates](https://developer.apple.com/documentation/updates/dockkit)
- [EnergyKit updates](https://developer.apple.com/documentation/updates/energykit)
- [File Provider updates](https://developer.apple.com/documentation/updates/fileprovider)
- [FinanceKit updates](https://developer.apple.com/documentation/updates/financekit)
- [Foundation updates](https://developer.apple.com/documentation/updates/foundation)
- [Foundation Models updates](https://developer.apple.com/documentation/updates/foundationmodels)
- [FSKit updates](https://developer.apple.com/documentation/updates/fskit)
- [Game Controller updates](https://developer.apple.com/documentation/updates/gamecontroller)
- [GameKit updates](https://developer.apple.com/documentation/updates/gamekit)
- [Group Activities updates](https://developer.apple.com/documentation/updates/groupactivities)
- [HealthKit updates](https://developer.apple.com/documentation/updates/healthkit)
- [Hypervisor updates](https://developer.apple.com/documentation/updates/hypervisor)
- [Journaling Suggestions updates](https://developer.apple.com/documentation/updates/journalingsuggestions)
- [LightweightCodeRequirements updates](https://developer.apple.com/documentation/updates/lightweightcoderequirements)
- [LiveCommunicationKit updates](https://developer.apple.com/documentation/updates/livecommunicationkit)
- [MapKit updates](https://developer.apple.com/documentation/updates/mapkit)
- [MapKitJS updates](https://developer.apple.com/documentation/updates/mapkitjs)
- [Matter updates](https://developer.apple.com/documentation/updates/matter)
- [MetricKit updates](https://developer.apple.com/documentation/updates/metrickit)
- [Network updates](https://developer.apple.com/documentation/updates/network)
- [Network Extension updates](https://developer.apple.com/documentation/updates/networkextension)
- [PaperKit updates](https://developer.apple.com/documentation/updates/paperkit)
- [PassKit updates](https://developer.apple.com/documentation/updates/passkit)
- [PencilKit updates](https://developer.apple.com/documentation/updates/pencilkit)
- [PHASE updates](https://developer.apple.com/documentation/updates/phase)
- [PhotoKit updates](https://developer.apple.com/documentation/updates/photokit)
- [ProximityReader updates](https://developer.apple.com/documentation/updates/proximityreader)
- [RealityKit updates](https://developer.apple.com/documentation/updates/realitykit)
- [SafariServices updates](https://developer.apple.com/documentation/updates/safariservices)
- [ScreenCaptureKit updates](https://developer.apple.com/documentation/updates/screencapturekit)
- [Security updates](https://developer.apple.com/documentation/updates/security)
- [Sensitive Content Analysis updates](https://developer.apple.com/documentation/updates/sensitivecontentanalysis)
- [SensorKit updates](https://developer.apple.com/documentation/updates/sensorkit)
- [ShazamKit updates](https://developer.apple.com/documentation/updates/shazamkit)
- [SiriKit updates](https://developer.apple.com/documentation/updates/sirikit)
- [Speech updates](https://developer.apple.com/documentation/updates/speech)
- [StoreKit updates](https://developer.apple.com/documentation/updates/storekit)
- [Swift updates](https://developer.apple.com/documentation/updates/swift)
- [Swift Charts updates](https://developer.apple.com/documentation/updates/swiftcharts)
- [SwiftData updates](https://developer.apple.com/documentation/updates/swiftdata)
- [SwiftUI updates](https://developer.apple.com/documentation/updates/swiftui)
- [Symbols updates](https://developer.apple.com/documentation/updates/symbols)
- [TipKit updates](https://developer.apple.com/documentation/updates/tipkit)
- [ThreadNetwork updates](https://developer.apple.com/documentation/updates/threadnetwork)
- [UIKit updates](https://developer.apple.com/documentation/updates/uikit)
- [User Notifications updates](https://developer.apple.com/documentation/updates/usernotifications)
- [Video Subscriber Account updates](https://developer.apple.com/documentation/updates/videosubscriberaccount)
- [Virtualization updates](https://developer.apple.com/documentation/updates/virtualization)
- [Vision updates](https://developer.apple.com/documentation/updates/vision)
- [Visual Intelligence updates](https://developer.apple.com/documentation/updates/visualintelligence)
- [watchOS updates](https://developer.apple.com/documentation/updates/watchos)
- [WeatherKit updates](https://developer.apple.com/documentation/updates/weatherkit)
- [WidgetKit updates](https://developer.apple.com/documentation/updates/widgetkit)
- [WorkoutKit updates](https://developer.apple.com/documentation/updates/workoutkit)
- [Xcode updates](https://developer.apple.com/documentation/updates/xcode)
- [XCUIAutomation updates](https://developer.apple.com/documentation/updates/xcuiautomation)
- [XPC updates](https://developer.apple.com/documentation/updates/xpc)

### Past releases

- [WWDC25](https://developer.apple.com/documentation/updates/wwdc2025)
- [WWDC24](https://developer.apple.com/documentation/updates/wwdc2024)
- [WWDC23](https://developer.apple.com/documentation/updates/wwdc2023)
- [WWDC22](https://developer.apple.com/documentation/updates/wwdc2022)
- [WWDC21](https://developer.apple.com/documentation/updates/wwdc2021)

## Anti-Patterns

- Do not infer iOS support from membership in this directory; it also includes macOS, drivers, web services, tools, and legacy APIs.
- Do not treat this source-checked topic map as a compiled implementation or assume every member is available on the landing page’s minimum OS.
- Do not copy a similarly named framework’s setup. Select the exact symbol, language, target type, and entitlement requirements from its source.

Generated from `docs/apple/technologies.json`; refresh with `python3 scripts/sync-apple-technologies.py --refresh`.

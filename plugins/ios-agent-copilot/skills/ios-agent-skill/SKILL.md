...
An optional unified stdio connection is available with `npx -y ios-agent-mcp@2.7.0 --project /absolute/path/to/app` (Node.js 20+). Its 35 tools include 12 read-only reviews, 8 reference tools, 14 simulator tools and app scaffolding. Simulator operations require macOS/Xcode; the unified server also exposes write/launch operations. Installing this Copilot plugin configures that connection automatically; do not add a duplicate. Copying only this SKILL.md into another client does not configure MCP. Use absolute app paths in review and simulator requests so the plugin installation directory is never mistaken for the app.

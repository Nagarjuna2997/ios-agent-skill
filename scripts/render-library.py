...
body='<p><a href="blog.html">← Back to all articles</a></p><header class="page-intro"><p class="eyebrow">Guided learning · Series roadmap</p><h1>From first prompt<br>to shipped app.</h1><p class="lead">A step-by-step Swift learning path, from opening Xcode to reviewing, testing and shipping an app.</p><p>Start with the available orientation lessons below. Later lessons are being developed in order. Hands-on tutorials require real-project checks, version records and failure notes before they are marked complete.</p></header><nav class="series-map" aria-label="Series levels">'+''.join('<a href="#level-'+str(l['number'])+'">'+str(l['number'])+' · '+html.escape(l['title'].split(' (')[0])+'</a>' for l in levels)+'</nav>'
for level in levels:

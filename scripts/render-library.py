...
body='<p><a href="blog.html">← Back to all articles</a></p><header class="page-intro"><p class="eyebrow">Guided learning · Series roadmap</p><h1>From first prompt<br>to shipped app.</h1><p class="lead">A step-by-step Swift learning path, from opening Xcode to reviewing, testing and shipping an app.</p><p>The lessons below are planned. Related reading is available now, with its own verification limits. New hands-on tutorials will be published only after their required real-project checks, version records and failure notes are complete.</p></header><nav class="series-map" aria-label="Series levels">'+''.join('<a href="#level-'+str(l['number'])+'">'+str(l['number'])+' · '+html.escape(l['title'].split(' (')[0])+'</a>' for l in levels)+'</nav>'
for level in levels:
 n=level['number'];body+='<section class="series-level" id="level-'+str(n)+'"><p class="eyebrow">Level '+str(n)+'</p><h2>'+html.escape(level['title'])+'</h2><ol class="series-lessons">'+''.join('<li><strong>'+x['number']+'</strong><span>'+html.escape(x['title'])+'</span><span class="lesson-status">Planned</span></li>' for x in level['lessons'])+'</ol>'

...
internals={'docs/mcp/registry.md','docs/mcp/vnext-analysis-tools.md','docs/tooling/issue-reporting.md','docs/orchestration/router.md','docs/orchestration/dynamic-workflows.md'}
cards=[];internal_cards=[];urls=[]
for source in sorted(paths):
 ...
 content='<div class="article-back"><a class="btn secondary" href="../library.html">← Back to all frameworks &amp; guides</a><a class="btn secondary" href="../blog.html">All articles →</a></div><article class="blog-article"><header class="page-intro"><p class="eyebrow">'+html.escape(category)+' · Reference guide</p><h1>'+html.escape(title)+'</h1><p class="lead">'+html.escape(desc)+'</p><p><a href="https://github.com/Nagarjuna2997/ios-agent-skill/blob/main/'+source+'">Read or improve the source</a> · <a href="../library.html">All guides</a></p></header><div class="article-body"><aside class="article-toc"><strong>On this page</strong>'+'<p><a href="#visual-overview">Visual overview: workflow and architecture</a></p>'+md.toc+'</aside>'+render_visuals(visual_plans[source])+body+'</div></article>'
 if source in internals:
  content=content.replace(' · Reference guide',' · Project internals').replace('href="../library.html">← Back to all frameworks &amp; guides','href="../library.html#project-internals">← Back to project internals')
 url='guides/'+slug+'.html';urls.append(url)
 (site/url).write_text(page(title,desc,content,url,1))
 (internal_cards if source in internals else cards).append('<article class="card" '+('data-internal' if source in internals else 'data-guide')+'><p class="eyebrow">'+html.escape(category)+'</p><h2><a href="'+url+'">'+html.escape(title)+'</a></h2><p>'+html.escape(', '.join(names) or category)+' </p></article>')
intro='<header class="page-intro"><p class="eyebrow">Learn from the source</p><h1>Frameworks. Features.<br>Practical decisions.</h1><p class="lead">Explore '+str(len(technologies))+' tracked technologies and '+str(len(cards))+' developer guides. Project maintenance and contributor references are listed separately below. These are maintained references, not a claim that every example has been compiled. Each guide retains its own availability and verification notes.</p><p><a href="blog.html">Start with a tested walkthrough →</a> · <a href="#project-internals">Project internals</a></p></header><label for="guide-search">Find a framework, feature or workflow</label><input class="library-search" id="guide-search" type="search" placeholder="Try SwiftUI, accessibility, assets or concurrency"><p id="guide-count" role="status" aria-live="polite"></p>'
(site/'library.html').write_text(page('Apple development guide library','Search repository guidance for Apple frameworks, SwiftUI, design assets, MCP tools and agent workflows.',intro+'<div class="grid blog-grid">'+''.join(cards)+'</div><section id="project-internals" aria-labelledby="internals-title"><h2 id="internals-title">Project internals</h2><p>Contributor and maintainer references for this project: publication, analyzer planning, issue reporting and agent coordination.</p><div class="grid blog-grid">'+''.join(internal_cards)+'</div></section><script src="library.js" defer></script>','library.html'))
...
print(f'Rendered {len(cards)} developer guides and {len(internal_cards)} project internals covering {len(technologies)} tracked technologies')
...
lesson_urls=series_renderer.render(root,page,base,blog['posts'])

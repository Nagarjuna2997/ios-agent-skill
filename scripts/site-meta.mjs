...
for(const p of pages){
 ...
 const image=meta(d,'og:image')[0]||new URL('assets/readme-hero.jpg',BASE).href;
 const article=group(p.file)==='Blog',tech=group(p.file)==='Docs';
 for(const [k,v] of Object.entries({title:p.title,description:p.description,url:p.url,type:article?'article':'website',image,site_name:NAME}))upsert('meta',{property:'og:'+k,content:v});
 ...
 let schema=p.file==='index.html'||group(p.file)==='Install guides'?{'@context':'https://schema.org','@type':'SoftwareSourceCode',name:NAME,codeRepository:REPO,programmingLanguage:['Swift','TypeScript'],license:'https://spdx.org/licenses/MIT.html',url:p.url}:article||tech?{'@context':'https://schema.org','@type':article?'Article':'TechArticle',headline:p.title,datePublished:p.datePublished||p.published,dateModified:p.modified,author:{'@type':'Person',name:'Nagarjuna'},mainEntityOfPage:p.url,url:p.url}:{'@context':'https://schema.org','@type':'CollectionPage',name:p.title,url:p.url};
